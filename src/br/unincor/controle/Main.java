...
public class Main {
	...
	public static void main(String[] args) {
		...
		List<Sanduiche> listaSanduiche = new ArrayList<Sanduiche>();
		Sanduiche s = new Sanduiche(null, null, null, null);
		
		
	while (gui.exibeMenuPrincipal() != 2) {
		if (gui.exibeMenuPrincipal() == 0) {
			
			String texto = gui.recebeTexto("Nome:");
			s.setNome(texto);
			Double preco = gui.recebeDouble("Preço:");
			s.setPreco(preco);
			int op = gui.recebeInteiro("duplo - 0 ** triplo - 1");
			if (op == 0) {
				s.setDobroRecheio(true);
			} else {
				s.setTrio(true);
			}
		
		}
		listaSanduiche.add(s);
		
		/*
			aqui vem o if da sobremesa
		*/
	}	
		
	
	if (gui.exibeMenuPrincipal() == 2) {
		gui.exibeMenuPagamento();
	}
	
	
	/*blablabla finalizaçao*/
